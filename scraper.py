...
# - [최종동기화] 기준주가/확정발행가 엔진에도 "병합셀 쪼개짐 스캔 방어막 해제" 로직 100% 동일 적용 (다른 코드는 절대 건드리지 않음)
# ========================================================== 
import os 
import re 
import json 
...
# (인벤티지랩, 지니너스 등 병합셀 오류를 100% 잡아내도록 수정 완료)
# ==========================================================
                ...
                for rr in range(r, min(r+4, R)): 
                    ...
                    if rr > r: 
                        ...
                        if len(curr_row_norm) != len(clean_next):  
                            if not any(t in curr_row_norm for t in target_kws): # 타겟 키워드가 있으면 통과
                                break 
            ...
            if any(t in combined_target for t in target_kws): 
                ...
                for rr in range(r, min(r+4, R)): 
                    ...
                    if rr > r: 
                        clean_next = _clean_label(curr_row_norm) 
                        if len(curr_row_norm) != len(clean_next):  
                            if not any(t in curr_row_norm for t in target_kws):
                                break 
                        if any(s in curr_row_norm for s in stop_kws): 
                            if not any(t in curr_row_norm for t in target_kws):
                                break 
