# ==========================================================
# #유상증자_코드V4.3 (Self-Healing 끝판왕)
# - 1200경 단위 숫자 오류 방지: 셀 내 숫자 병합(Squash) 차단
# - 날짜 텍스트 오류 방지: 값에 숫자가 없으면 날짜로 취급 안 함
# - 발행가 "6" 오류 방지: 30 이하의 단순 숫자는 인덱스로 간주하고 스킵
# - 기존 상장시장 데이터 증발 방지 보호망 적용
# ==========================================================
import os
import re
import json
            ...
            for c in range(C):
                if _clean_label(arr[r][c]) in cand_clean:
                    ...
                    for v in [v for v in checks + row_vals if v.lower() != "nan"]:
                        ...
                        # [오류 방지] 30 이하의 숫자는 항목 인덱스(1, 2, 6 등)일 가능성이 높으므로 값으로 취급 안 함
    ...
    for df in dfs:
        ...
        for r in range(arr.shape[0]):
            ...
            if all(k in _norm("".join(row)) for k in keys):
                # 각 셀마다 _max_int_in_text를 사용하여 셀 병합 오류(Squash) 차단
                row_max = 0
                for cell in row:
        ...
        for r in range(arr.shape[0]):
            ...
            for k, std_name in keys_map.items():
                if _norm(k) in row_joined:
                    # [오류 방지] 전체 행을 뭉뚱그리지 않고 각 '셀'별로 가장 큰 숫자를 찾음
                    row_max = 0
                    for cell in row:
                        amt = _max_int_in_text(cell)
                        if amt and amt >= 100:
...
def parse_rights_issue_record(dfs, t: Target, corr_after, html_raw) -> dict:
    ...
    # [보호망] 기존에 적혀있던 상장시장이 날아가지 않도록 철저히 체크
    ...
    # [오류 방지] 날짜 데이터에 숫자가 한 개도 없으면 텍스트(정정 사유 등)로 간주하고 무시함
...
def run():
    ...
    for row in values[1:]:
        ...
        # [감지 조건 최종 조율] 이상한 숫자(길이가 너무 길거나 짧음), 이상한 날짜 감지
        needs_fix = (
            not fund or "(원)" in fund or
            not price or (price.isdigit() and len(price) <= 2) or
            len(fund_amt.replace(",", "").replace(".", "")) >= 15 or # 1200경 오류 감지
            not market or
