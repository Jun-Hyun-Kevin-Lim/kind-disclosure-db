...
# - [개선] 신규발행주식수 추출 시 '증자전', '기발행' 주식수가 덮어쓰는 버그 원천 차단
# ==========================================================
import os
import re
import json
...
def extract_issue_shares_and_type(dfs: List[pd.DataFrame], corr_after: Dict[str, str]) -> Tuple[Optional[int], str]:
    ...
    for df in dfs:
        ...
        for r in range(R):
            ...
            # [픽스] "증자전", "발행주식총수", "기발행", "비율" 등이 포함된 행은 절대 신규발행주식수가 아님! 무조건 건너뜀
            if any(bad in row_joined for bad in ["증자전", "기발행", "총수", "감자전", "비율"]):
                continue
            
            if any(kw in row_joined for kw in ["신주의종류", "발행예정주식", "보통주", "종류주", "우선주", "기타주"]):
                if any(d in row_joined for d in ["년", "월", "일", "예정일", "기일"]): continue
                
                # [픽스] 단순히 큰 값을 덮어쓰는 것(amt > best_amt)이 아니라 해당 행에 있는 가장 우측 최신 값을 채택
                valid_amts = []
                for cell in arr[r]:
                    amt = _max_int_in_text(str(cell))
                    if amt and amt > 0:
                        valid_amts.append(amt)
                
                if valid_amts:
                    found_amt = valid_amts[-1]
                    if found_amt > 0:
                        best_amt = found_amt
                        ...
                        # 명확한 대상(신주의 종류와 수)을 찾았다면, 더 이상 아래쪽의 엉뚱한 행을 뒤져서 덮어쓰지 않도록 즉시 탐색 중단
                        if any(kw in row_joined for kw in ["신주의종류", "발행예정주식"]):
                            break
        
        # 값을 찾았으면 다음 표로 넘어가지 않음
        if best_amt > 0:
            break
