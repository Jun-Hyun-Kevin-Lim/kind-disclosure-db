...
# - [완벽해결] HTML rowspan 특성 활용하여 어설픈 다중행 탐색 폐기 및 보통주/기타주 독립 누적 합산 로직 적용 (압타머사이언스 해결)
# - [완벽해결] 기준주가에서 할인율(%), 연도(2026)를 가격으로 오인하는 버그 완벽 차단 (인벤티지랩 해결)
# ==========================================================
import os
import re
import json
...
# ==========================================================
# [수술부위 1] 기준주가 및 발행가액 정밀 타격 엔진 (인벤티지랩, 지니너스 해결)
# - 연도 및 % 숫자를 철저히 지워버리고, 해당 구역(Row)만 집중 공략
# ==========================================================
...
# ==========================================================
# [수술부위 2] 보통주 + 기타주 완벽 분할 합산 스나이퍼 (압타머사이언스 등 해결)
# - HTML rowspan의 특성을 활용하여 독립된 행에서 값을 누적 합산
# ==========================================================
def get_shares_info_by_exact_section(dfs: List[pd.DataFrame], corr_after: Dict[str, str], section_keywords: List[str], stop_keywords: List[str]) -> Tuple[Optional[int], int, int]:
    ...
    for df in dfs:
        ...
        for r in range(R):
            ...
            # 해당 행이 찾는 항목("증자전" 등)이라면 무조건 값을 읽어 합산함
            if any(t in row_str_norm for t in section_keywords):
                ...
                if row_max > 0:
                    has_com = "보통" in row_str_norm
                    has_oth = any(k in row_str_norm for k in ["기타", "종류", "우선", "상환전환"])
                    has_tot = any(k in row_str_norm for k in ["합계", "총계", "총수", "계"])
                    
                    # 같은 줄에 둘 다 있으면 정규식으로 쪼개서 넣기
                    if has_com and has_oth:
                        m_com = re.search(r'보통[^0-9]*((?:\d{1,3}[,.]?)+\d{3,})', row_str_norm)
                        if m_com:
                            val = int(re.sub(r'[,.]', '', m_com.group(1)))
                            if val > 100: cv = max(cv, val)
                        m_oth = re.search(r'(?:기타|종류|우선|상환전환)[^0-9]*((?:\d{1,3}[,.]?)+\d{3,})', row_str_norm)
                        if m_oth:
