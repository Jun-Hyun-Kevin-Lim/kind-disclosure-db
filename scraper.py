...
# - [개선] 증자전 주식수 "보통주식 + 종류주식(기타)" 완벽 분리 합산 엔진 탑재 
# - [최종개선] "발행상품" 컬럼 개선: 텍스트가 아닌 실제 수량(Amt) 기반으로 추적하여 "보통주식" / "우선주식" 정확히 양분 
# - [긴급개선] 인벤티지랩, 지니너스 기준주가/발행가액 오류 해결 (연도 2026, 할인율 7.0% 등 가짜 숫자 완벽 차단 필터 적용)
# - [최종동기화] 신규발행주식수 엔진을 '증자전 주식수(get_prev_shares_sum)'에서 검증된 완벽한 블록 스캔/합산 로직과 100% 동일하게 이식
# ========================================================== 
import os 
import re 
import json 
...
# ========================================================== 
# [최종동기화] 신규발행주식수 전용 엔진
# - 증자전 주식수(get_prev_shares_sum)의 완벽한 텍스트 스나이핑 로직을 100% 동일하게 이식
# ========================================================== 
        ...
        for k, v in corr_after.items(): 
            ...
            if any(t in k_norm for t in target_kws): 
                if not any(s in k_norm for s in stop_kws): 
                    v_norm = _norm(v) 
                    v_norm_clean = re.sub(r'202\d[년월일\.]?', '', v_norm) 
                    cv, ov, tv = 0, 0, 0 
                    
                    m_com = re.findall(r'보통[^0-9]*?((?:\d{1,3}[,.]?)+\d{3,})', v_norm_clean) 
                    if m_com: cv = max([int(re.sub(r'[,.]', '', x)) for x in m_com]) 
                    m_oth = re.findall(r'(?:기타|종류|우선|상환전환)[^0-9]*?((?:\d{1,3}[,.]?)+\d{3,})', v_norm_clean) 
                    if m_oth: ov = max([int(re.sub(r'[,.]', '', x)) for x in m_oth]) 
                    m_tot = re.findall(r'(?:합계|총계|총수|계)[^0-9]*?((?:\d{1,3}[,.]?)+\d{3,})', v_norm_clean) 
                    ...
                    if best_amt > 0: 
                        ...
                        elif any(x in v_norm for x in ["우선", "종류", "기타", "상환전환"]): 
    ...
    for df in dfs: 
        ...
        for r in range(R): 
            ...
            if any(t in combined_target for t in target_kws): 
                ...
                block_text_clean = re.sub(r'202\d[년월일\.]?', '', block_text) 
                block_text_clean = re.sub(r'\d+(?:\.\d+)?%', '', block_text_clean) 
                
                cv, ov, tv = 0, 0, 0 
                m_com = re.findall(r'보통[^0-9]*?((?:\d{1,3}[,.]?)+\d{3,})', block_text_clean) 
                if m_com: cv = max([int(re.sub(r'[,.]', '', x)) for x in m_com]) 
                m_oth = re.findall(r'(?:기타|종류|우선|상환전환)[^0-9]*?((?:\d{1,3}[,.]?)+\d{3,})', block_text_clean) 
                if m_oth: ov = max([int(re.sub(r'[,.]', '', x)) for x in m_oth]) 
                m_tot = re.findall(r'(?:합계|총계|총수|계)[^0-9]*?((?:\d{1,3}[,.]?)+\d{3,})', block_text_clean) 
                if m_tot: tv = max([int(re.sub(r'[,.]', '', x)) for x in m_tot]) 
                ...
                # 보통, 기타 등의 라벨이 없이 그냥 숫자만 표기된 경우 (최후의 보루)
                if best_amt == 0: 
                    text_for_nums = block_text_clean 
                ...
                if best_amt > 0: 
                    ...
                    elif any(x in block_text for x in ["우선", "종류", "기타", "상환전환"]): 
...
# ========================================================== 
# [유지] 증자전 주식수 전용: 보통주 + 종류주식 완벽 합산 엔진 (기존 성공 로직)
# ========================================================== 
